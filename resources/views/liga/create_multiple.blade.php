...
@section('content')
    ...
    <form action="{{ url('multiple-match') }}" method="POST">
        @csrf
        <div id="dynamicAddRemove">
            <div class="d-flex">
                <div class="col mt-2">
                    <label for="clubName"><strong> Home Club Name</strong></label>
                    <select name="club[0]" class="custom-select  mb-1">
                ...
                <div class="col mt-2">
                    ...
                    <select name="club[1]" class="custom-select  mb-1">
                        <option selected value="">Select Club</option>
                        @foreach ($club as $key => $value)
                            <option value={{ $value->id }}>{{ $value->name }}</option>
                        @endforeach
    ...
    <script type="text/javascript">
        ...
        $("#addMatch").click(function() {
            ...
            $('#dynamicAddRemove').append(
                '<div class = "d-flex child'+i+'" id = "dynamicAddRemove" ><div class = "col mt-2" > <label><strong> Home Club Name </strong></label> <select name = "club[' +
                i +
                ']"class = "custom-select  mb-1" ><option selected > Select Club </option> @foreach ($club as $key => $value)<option value = {{ $value->id }} > {{ $value->name }} </option> @endforeach </select> @foreach ($errors->get("club.'+i+'") as $msg) <p class="text-danger small">{{ $msg }}</p> @endforeach </div> <span class = "mt-5" > - </span> <div class = "col mt-2" ><label><strong> Away Club Name </strong></label> <select name = "club[' +
                away +
                ']" class = "custom-select  mb-1" ><option selected > Select Club </option> @foreach ($club as $key => $value)<option value = {{ $value->id }} > {{ $value->name }} </option> @endforeach </select> @foreach ($errors->get("club.'+i+'") as $msg) <p class="text-danger small">{{ $msg }}</p> @endforeach </div> <div class = "col mt-2" ><label for = "home_score"><strong>Home Score Club</strong></label> <input name = "scor[' +
                i +
                ']"type = "text"class = "form-control"id = "home_score" > @foreach ($errors->get("scor.'+i+'") as $msg) <p class="text-danger small">{{ $msg }}</p> @endforeach </div> <span class = "mt-5" > - </span> <div class = "col mt-2" ><label for = "away_score"><strong>Away Score Club</strong></label> <input name = "scor[' +
                away +
